...
import { KpiSummary, ReportTabs } from "@/features/admin/components";
...
import type { ReportCrewPerformance } from "@/features/admin/types";
import { useTranslation } from "@/i18n";
import { cn } from "@/lib/utils";
import {
  toISODate,
  currentPeriod as currentMonth,
  formatPeriodLabel,
  formatRupiah,
  formatTurnaround,
...
export default function ReportsPage() {
  ...
  const { report, isLoading, refetch } = useAdminReport(
  ...
  const statusEntries = useMemo(
    () => Object.entries(report?.bookings.byStatus ?? {}),
    [report?.bookings.byStatus],
  );
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <span className='font-medium'>{row.original.crewName}</span>
        ),
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className='flex items-center justify-end gap-2'>
              ...
              <span
                className={cn("font-mono text-xs", reliabilityTextColor(score))}
              >
                {score.toFixed(1)}%
              </span>
            </div>
            ...
            <div className='space-y-2 px-4 py-3'>
              ...
              >
                <SelectTrigger
                  className={cn(
                    "h-8 w-full text-xs",
                    siteId ? "border-primary/40 font-medium" : "",
                  )}
                >
                  <SelectValue />
                </SelectTrigger>
        ...
        <div className='rounded-xl border border-amber-200 bg-amber-50 dark:border-amber-900/40 dark:bg-amber-950/20'>
          <div className='flex flex-wrap items-center gap-4 p-4'>
            ...
            <div className='flex gap-6 text-right'>
              ...
              <div>
                <p className='text-xs text-amber-600/70 dark:text-amber-500/60'>
                  {t("reports.tipCard.crewCount")}
                </p>
                <p className='mt-0.5 text-sm font-semibold text-amber-800 dark:text-amber-300'>
                  {crewSummary.summary.crewCount}{" "}
                  {t("reports.tipCard.crewCountUnit")}
                </p>
              </div>
            </div>
