...
export default function TipsReportPage() {
  ...
  async function handleExportCSV(disbursementId: string, disbursementPeriod: string) {
    ...
    try {
      const res = await adminApi.get<Blob>(`/v1/admin/disbursements/${disbursementId}/export`, {
        responseType: "blob",
      });
      const blob = res.data;
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Ringkasan tips crew dan riwayat disbursement</p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <h2 className="text-sm font-semibold text-foreground">Ringkasan Tips per Crew</h2>
          ...
          <table className="w-full text-sm">
            <thead className="border-b border-border bg-muted/50">
              <tr>
                ...
                <th className="px-4 py-3 text-right font-medium text-muted-foreground">Tip Masuk</th>
              ...
              {isSummaryLoading ? (
                ...
                crewSummary.crews.map((crew) => (
                  <tr key={crew.crewId} className="hover:bg-muted/30">
                    ...
                    <td className="px-4 py-3 text-right font-mono text-xs">{crew.tipCount}</td>
      ...
      <div className="space-y-3">
        <h2 className="text-sm font-semibold text-foreground">Riwayat Disbursement</h2>
            ...
            <table className="w-full text-sm">
              <thead className="border-b border-border bg-muted/50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-center font-medium text-muted-foreground">Export</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {disbursements.map((d) => (
