...
function formatElapsedShort(seconds: number, t: ReturnType<typeof useTranslation<"admin">>["t"]): string {
...
function DashboardSiteCard({ site, onBookingClick }: DashboardSiteCardProps) {
    ...
    <div className="rounded-xl border border-border bg-card shadow-sm flex flex-col">
      ...
      <div className="flex items-center justify-between gap-3 p-4 border-b border-border">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <h2 className="text-sm font-semibold text-foreground truncate">{site.siteName}</h2>
        </div>
        <Button
...
function DashboardContent() {
  ...
  const [clickedBooking, setClickedBooking] = useState<AdminQueueBooking | null>(null);
  const { t } = useTranslation("admin");
