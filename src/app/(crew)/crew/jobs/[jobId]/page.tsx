...
import {
  useCrewJob,
  useEtaExpired,
  useSupervisorContact,
} from "@/features/crew/hooks";
import {
  EtaCountdown,
  NeedsHelpModal,
  TimeExtensionControl,
} from "@/features/crew";
import { BOOKING_STATUS_TONES } from "@/features/customer/types";
import { TIMER_HIDDEN_STATUSES } from "@/features/crew/types";
...
export function CrewJobDetailPage() {
  ...
  if (error || !job) {
    return (
      <main className='mx-auto flex min-h-[calc(100dvh-44px)] max-w-md flex-col items-center justify-center gap-5 px-4 text-center'>
        ...
        <Button variant='outline' size='md' onClick={goBackToQueue}>
          {t("job.backToQueue")}
        </Button>
      </main>
    );
  ...
  const beforePhotoKinds = [
    "before_front",
    "before_back",
    "before_left",
    "before_right",
  ];
  const hasAllBeforePhotos = beforePhotoKinds.every((k) =>
  ...
  return (
    <>
      ...
      <main className='mx-auto max-w-md px-4 pb-28 pt-4'>
        ...
        {/* ── ETA countdown ───────────────────────────────────────────── */}
        {job.etaEndsAt && !TIMER_HIDDEN_STATUSES.has(job.status) ? (
          <div className='mb-4'>
            ...
            {job.status === "LOCATED" && (
              <div className='flex flex-col gap-2'>
                <Button
                  ...
                  disabled={expired}
                  onClick={() =>
                    router.push(`/crew/jobs/${jobId}/before-photos`)
                  }
                  suffix={<ArrowRight className='h-5 w-5' />}
              ...
              <div className='flex flex-col gap-2'>
                <Button
                  ...
                  onClick={() =>
                    router.push(
                      hasAllBeforePhotos
                        ? `/crew/jobs/${jobId}/wash`
                        : `/crew/jobs/${jobId}/before-photos`,
                    )
                  }
                  suffix={<ArrowRight className='h-5 w-5' />}
                >
