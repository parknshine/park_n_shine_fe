...
import api, { ApiContractError } from "@/lib/axios-admin";
...
import {
  StatusOverrideModal,
  hasValidStatusOverrideTransitions,
} from "./status-override-modal";
...
import {
  formatAuditDetail,
  formatAuditAction,
} from "@/features/admin/utils/format-audit-detail";
...
export function BookingDetailDrawer({
  ...
  const { setNotificationSent, isSettingNotificationSent } = useBookingActions(
    bookingId ?? "",
  );
  ...
  async function handleTimeExtensionRespond(
    approved: boolean,
    minutes: number,
  ) {
    if (!bookingId) return;
    ...
    try {
      await api.post(
        `/v1/admin/bookings/${bookingId}/time-extension-request/respond`,
        {
          approved,
          ...(approved ? { minutes } : {}),
        },
      ...
      toast.success(
        approved
          ? t("timeExtension.approveSuccess")
          : t("timeExtension.rejectSuccess"),
      );
      onActionSuccess();
    } catch (error) {
      const alreadyResolved =
        ...
        (error.httpStatus === 404 ||
          error.message?.includes("No pending time extension"));
      if (alreadyResolved) {
        onActionSuccess();
      } else {
        toast.error(
          approved
            ? t("timeExtension.approveError")
            : t("timeExtension.rejectError"),
        );
      }
    } finally {
  ...
  async function handleExtendTime() {
    ...
    try {
      const minutes =
        typeof extendMinutes === "number" && extendMinutes > 0
          ? extendMinutes
            ...
            <>
              ...
              <section>
                ...
                <dl className='grid grid-cols-[auto_1fr] gap-x-4 gap-y-2 text-sm'>
                  ...
                  <dt className='text-muted-foreground'>
                    {t("drawer.estimatedReady")}
                  </dt>
                  <dd className='font-medium text-foreground'>
                    {booking.estimatedReadyAt
                      ? new Date(booking.estimatedReadyAt).toLocaleString(
                          "id-ID",
                        )
                      : "—"}
                  </dd>
                    ...
                    {booking.statusHistory.map((entry, i) => (
                      <li key={i} className='flex flex-col gap-1 text-sm'>
                        <div className='flex items-center gap-3'>
                          <StatusBadge
                            tone={
                              BOOKING_STATUS_TONES[entry.status] ?? "neutral"
                            }
                          >
                            {entry.status}
                          </StatusBadge>
                    ...
                    {booking.media.map((m) => (
                      <div key={m.id} className='space-y-1'>
                        {m.url ? (
                          ...
                          <div className='h-24 w-full rounded-md border border-border bg-muted' />
                        )}
              ...
              <section>
                ...
                <div className='flex flex-col gap-2'>
                  ...
                  {showExtend && (
                    <div className='rounded-lg border border-border bg-muted/40 p-3 space-y-2'>
                      <p className='text-xs font-semibold text-foreground'>
                        {t("drawer.extendTimeTitle")}
                      </p>
                      <div>
                        <label className='text-xs text-muted-foreground'>
                          {t("drawer.extendTimeMinutes")}
                        </label>
                        <input
                          type='text'
                          inputMode='numeric'
                  ...
                  {booking?.pendingTimeExtension?.status === "PENDING" &&
                    !["CLOSED", "CANCELLED", "EXPIRED"].includes(
                      booking.status,
                    ) && (
                        ...
                        <div className='flex items-center gap-2'>
                          ...
                          <Button
                            ...
                            disabled={
                              ...
                              !pendingExtMinutes ||
                              Number(pendingExtMinutes) < 1
                            }
                            onClick={() =>
                              handleTimeExtensionRespond(
                                true,
                                Number(pendingExtMinutes),
                              )
                            }
                          >
